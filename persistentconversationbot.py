...
def handle_question(update: Update, context: CallbackContext) -> int:
    ...
    if (len(possible_answers) == 0):
        ...
        if (possible_answers[0]['tag'] in directly_answered):
            reply_text = possible_answers[0]['responses'][0]
            update.message.reply_text(reply_text)
            return ASKING
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('start', start),MessageHandler(
                     Filters.text  & ~(Filters.regex(r'(?i)^(bye|good bye|au revoir)$') | Filters.command('stop')), handle_question
                )],
        states={
            ASKING: [
                MessageHandler(
            ...
            TYPING_FIRST_CHOICE: [
                MessageHandler(
                    Filters.text & ~(Filters.regex(r'(?i)^(bye|good bye|au revoir)$') | Filters.command('stop')), handle_first_choice
                )
            ],
            TYPING_SECOND_CHOICE: [
                MessageHandler(
                    Filters.text & ~(Filters.regex(r'(?i)^(bye|good bye|au revoir)$') | Filters.command('stop')), handle_second_choice
                )
            ],
        },
...
def respond(user_input):
    ...
    sentence_bag_of_words = sentence_bag_of_words.reshape(-1, 1, 210)
